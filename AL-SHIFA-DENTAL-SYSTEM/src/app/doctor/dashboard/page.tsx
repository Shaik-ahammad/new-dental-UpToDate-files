...
export default function DoctorDashboard() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 font-sans">
      ...
      {/* --- 2. KPI CARDS (Royal Emerald Theme) --- */}
      ...
      <div className="grid gap-6 md:grid-cols-7">
        ...
        {/* LEFT: Today's Schedule (Takes 4/7 space) */}
        ...
        {/* RIGHT: AI Assistant & Inventory (Takes 3/7 space) */}
        <div className="md:col-span-3 space-y-6">
            ...
            <Card className="border-0 shadow-xl bg-slate-900 text-white relative overflow-hidden h-fit">
                ...
                <CardContent className="space-y-4 pt-6 relative z-10">
                    ...
                    <div className="p-4 bg-white/5 rounded-xl border border-white/10 hover:bg-white/10 transition-colors cursor-pointer group">
                        <p className="text-sm font-bold text-emerald-400 mb-1 flex items-center gap-2 group-hover:text-emerald-300 transition-colors">
                            <Clock className="w-3 h-3" /> Smart Schedule
                        </p>
                        <p className="text-xs text-slate-300 leading-relaxed">
                            You have a 2-hour gap between 12:00 PM and 02:00 PM. 
                            <br/>Action: Open for walk-ins?
                        </p>
                        <div className="mt-3 flex gap-2">
                            <button className="text-[10px] font-bold bg-emerald-600 hover:bg-emerald-500 text-white px-3 py-1.5 rounded-full transition-colors">
                                Accept
                            </button>
                            <button className="text-[10px] font-bold bg-white/10 hover:bg-white/20 text-slate-300 px-3 py-1.5 rounded-full transition-colors">
                                Dismiss
                            </button>
                        </div>
                    </div>
            ...
            {/* Inventory Alerts (Only if data exists) */}
            {stats?.inventory_alerts && stats.inventory_alerts.length > 0 && (
                <Card className="border-amber-200 bg-amber-50 shadow-sm animate-in fade-in zoom-in-95 duration-500">
                    ...
                    <CardContent className="p-4 space-y-2">
                        ...
                        <Link href="/doctor/inventory" className="block mt-2">
                            <Button variant="link" size="sm" className="text-amber-700 p-0 h-auto text-xs w-full justify-end hover:text-amber-900">
                                Restock Now ->
                            </Button>
                        </Link>
                    </CardContent>
